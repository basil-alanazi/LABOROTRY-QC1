...
// Codes that already exist get their times/color updated (upsert) rather
// than being silently skipped, so re-uploading a corrected file always works.
export default function ShiftTemplateImport({ existingCodes, onApply }) {
  ...
  function confirmApply() {
    const toApply = parsed.filter((r) => r.code.trim());
    const newCount = toApply.filter((r) => !r.existing).length;
    const updateCount = toApply.filter((r) => r.existing).length;
    onApply(toApply);
    setApplyMsg(
      updateCount > 0
  ...
  return (
    <div style={{ marginBottom: 10 }}>
      ...
      {applyMsg && !parsed && (
        <div style={{ display: "flex", alignItems: "flex-start", gap: 6, fontSize: 12, color: "#2F6B4F", marginTop: 6 }}>
      ...
      {parsed && parsed.length > 0 && (
        <div style={{ background: "#FBF8F0", border: "1px solid #E8DCC0", borderRadius: 8, padding: 10, marginTop: 10 }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 6, maxHeight: 340, overflowY: "auto" }}>
            {parsed.map((r, i) => (
              <div key={i} style={{ display: "flex", gap: 5, alignItems: "center", background: r.existing ? "#FBF3DF" : "#fff", borderRadius: 6, padding: 6 }}>
