...
Infection Control site for the hospital's IC team: Daily IC Rounds, Nursing Daily Rounds,
Quality Daily Rounds, Employee Health (vaccinations/screenings), Suspected/Confirmed Cases,
renewal Trackers (Baladiya license, policy, culture, agreements), Stock Requests, and internal
Messages — each its own module, described below.
...
- **Ward Staff** — self-service Stock Requests for their own department (see item 6 below). Can instead be marked **"Employee Health access only"**, **"Nursing Daily Rounds access only"**, or **"Quality Daily Rounds access only"** (Settings → User Accounts) for an account that should only ever see that one page — e.g. a doctor's account, or the specific staff running Nursing/Quality's own daily rounds — in place of the usual stock-only view
