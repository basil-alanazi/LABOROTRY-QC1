...
9. **"Employee Health"** (Owner/IC only): mirrors the clinic's real Employee Clinic tracking sheet. Employee roster keeps employee #/file #/iqama #/DOB/phone/department plus a "Kitchen Staff" flag (kitchen staff automatically get extra items). The **Employee Clinic** tab is where the work happens per employee: an investigation status (doctor review due / sample not yet given / review done, covering the standard serology panel), a PPD status with test date and auto-calculated next-due date, a Stool & Urine Test status for Kitchen Staff (auto next-due +6 months), ICN remarks, and a vaccination checklist — tick which vaccine the doctor requested (Hep B, MMR, Varicella, Tetanus, Influenza, plus Meningococcal/Typhoid/Hepatitis A for Kitchen Staff), then log each dose with its date and batch number; the next dose's due date is calculated automatically from the 1st dose date (e.g. Hep B: 0 / +1 month / +6 months). An "Overdue & Missing" view rolls all of this up (PPD, stool/urine, and every partially-completed vaccine series) across every employee, and everything exports to Excel/PDF (roster, investigation & PPD status, and the full per-dose vaccination log)
