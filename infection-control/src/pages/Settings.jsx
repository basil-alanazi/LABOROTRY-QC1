...
export default function Settings() {
  ...
  async function saveChecklist(checklist) {
    await supabase
      ...
      .eq("id", checklist.id);
    flash(`تم حفظ ${checklist.name_ar}`);
  }
  ...
  return (
    <div className="flex flex-col gap-8">
      <div>
        ...
        <p className="text-sm text-slate-500">إدارة الأقسام وقوائم التدقيق والحسابات.</p>
      </div>

      {message && <p className="rounded-lg bg-emerald-50 px-3 py-2 text-sm text-emerald-700">{message}</p>}

      <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        <h2 className="mb-4 text-sm font-semibold text-slate-700">الأقسام</h2>
        ...
        <div className="flex gap-2">
          <input className="input" value={newDept} onChange={(e) => setNewDept(e.target.value)} placeholder="اسم قسم جديد" />
      ...
      <section className="flex flex-col gap-4">
        <h2 className="text-sm font-semibold text-slate-700">قوائم التدقيق (الحزم)</h2>
        {checklistTypes.map((c) => (
          <div key={c.id} className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
            ...
            <label className="mb-1 block text-xs font-medium text-slate-500">بنود الحزمة (سطر لكل بند)</label>
            <textarea
      ...
      {isOwner && accounts && (
        <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
          <h2 className="mb-4 text-sm font-semibold text-slate-700">الحسابات</h2>
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <AccountFields
              title="فريق الأقسام (إدخال فقط)"
  ...
  return (
    <div className="rounded-xl border border-slate-100 p-4">
      ...
      <div className="flex flex-col gap-2">
        <input className="input" value={username} onChange={(e) => onChange(e.target.value, password)} placeholder="اسم المستخدم" />
        <input className="input" value={password} onChange={(e) => onChange(username, e.target.value)} placeholder="كلمة المرور" />
      </div>
    </div>
  );
}
