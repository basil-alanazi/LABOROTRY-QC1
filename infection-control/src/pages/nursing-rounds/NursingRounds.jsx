...
const NURSING_ROUND_ATTACHMENTS_BUCKET = "nursing-round-attachments";

function attachmentUrl(path) {
  if (!path) return null;
  return supabase.storage.from(NURSING_ROUND_ATTACHMENTS_BUCKET).getPublicUrl(path).data.publicUrl;
}
...
export default function NursingRounds() {
  ...
  const departments = config?.nursing_round_departments ?? [];

  async function loadAll() {
    setLoading(true);
    const { data } = await fetchAllRows((from, to) =>
      supabase.from("nursing_rounds").select("*").eq("deleted", false).order("date", { ascending: false }).order("created_at", { ascending: false }).range(from, to)
    );
    setRounds(data ?? []);
    setLoading(false);
  }
  ...
  async function uploadFile(file, department) {
    ...
    const { error } = await supabase.storage.from(NURSING_ROUND_ATTACHMENTS_BUCKET).upload(path, file);
  ...
  async function handleSubmit(e) {
    ...
    const { error } = await supabase.from("nursing_rounds").insert({
      date: form.date,
  ...
  async function toggleStatus(r) {
    await supabase.from("nursing_rounds").update({ status: r.status === "closed" ? "open" : "closed" }).eq("id", r.id);
    loadAll();
  }
  ...
  async function addAttachment(r, files) {
    try {
      ...
      await supabase.from("nursing_rounds").update({ attachments: next }).eq("id", r.id);
    } catch (err) {
  ...
  async function removeRound(r) {
    ...
    await supabase.from("nursing_rounds").update({ deleted: true, deleted_by: session?.username, deleted_at: new Date().toISOString() }).eq("id", r.id);
    loadAll();
  }

  function exportExcel() {
    downloadExcel(`infection-control-nursing-rounds-${todayStr()}`, [{ name: "Nursing Rounds", headers: REPORT_HEADERS, rows: visibleRounds.map(toReportRow) }]);
  }

  function exportPdf() {
    downloadPdf(`infection-control-nursing-rounds-${todayStr()}`, "Infection Control — Nursing Daily Rounds", [{ headers: REPORT_HEADERS, rows: visibleRounds.map(toReportRow) }]);
  }
  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-xl font-bold text-slate-800">Nursing Daily Rounds</h1>
