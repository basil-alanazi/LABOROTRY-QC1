...
export default function StockRequests() {
  ...
  async function loadDailyChecks() {
    ...
    const { data } = await supabase
      .from("stock_daily_checks")
      .select("*")
      .eq("date", checkDate)
      .in(
        "item_id",
        dailyCheckItems.map((i) => i.id)
      );
    const map = {};
    for (const item of dailyCheckItems) {
      map[item.id] = (data ?? []).find((r) => r.item_id === item.id) || emptyDailyCheck(item.id);
    }
    setChecksByItem(map);
  ...
  useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [tab, checkDate, activeDepartment, items.length]);

  function updateCheckField(itemId, patch) {
    setChecksByItem((prev) => ({ ...prev, [itemId]: { ...(prev[itemId] || emptyDailyCheck(itemId)), ...patch } }));
  }
        ...
        <div className="flex flex-col gap-4 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
          ...
          {activeDepartment && (
            <div className="overflow-x-auto rounded-xl border border-slate-200">
                      ...
                      <Fragment key={s.key}>
                        <th className="px-2 py-1.5 text-center font-normal">
                          <button
                            ...
                            title="Mark every item checked for this shift"
                          ...
                          <Fragment key={s.key}>
                            <td className="px-2 py-1.5 text-center">
                              <input
                                ...
                                onChange={(e) => saveCheck(item, { [`${s.key}_checked`]: e.target.checked })}
                              />
                            </td>
                            <td className="px-2 py-1.5">
                              <input
                                className="input-cell w-24"
                                value={row[`${s.key}_by`] || ""}
                                onChange={(e) => updateCheckField(item.id, { [`${s.key}_by`]: e.target.value })}
              ...
              {!loadingChecks && dailyCheckItems.length === 0 && (
                <p className="p-6 text-center text-sm text-slate-400">No items in {activeDepartment}'s stock catalog yet.</p>
              )}
