              ...
              <div style={{ marginTop: 10, background: "#F7F9F8", borderRadius: 7, padding: 10 }}>
                ...
                {(p.analytes || []).map((a) => (
                  <div key={a.name} style={{ display: "flex", gap: 6, alignItems: "center", marginBottom: 6 }}>
                    ...
                    <input placeholder="Low" type="number" value={baselineValues[a.name]?.rangeLow ?? ""} onChange={(e) => setBaselineValues((b) => ({ ...b, [a.name]: { mean: "", sd: "", rangeHigh: "", target: "", ...b[a.name], rangeLow: e.target.value } }))} style={{ ...inputStyle, width: 70, marginTop: 0 }} />
                    <input placeholder="High" type="number" value={baselineValues[a.name]?.rangeHigh ?? ""} onChange={(e) => setBaselineValues((b) => ({ ...b, [a.name]: { mean: "", sd: "", rangeLow: "", target: "", ...b[a.name], rangeHigh: e.target.value } }))} style={{ ...inputStyle, width: 70, marginTop: 0 }} />
                    <span style={{ fontSize: 10, color: "#C7D1CE" }}>or</span>
                    <input placeholder="Mean" type="number" value={baselineValues[a.name]?.mean ?? ""} onChange={(e) => setBaselineValues((b) => ({ ...b, [a.name]: { sd: "", rangeLow: "", rangeHigh: "", target: "", ...b[a.name], mean: e.target.value } }))} style={{ ...inputStyle, width: 80, marginTop: 0 }} />
                    <input placeholder="SD" type="number" value={baselineValues[a.name]?.sd ?? ""} onChange={(e) => setBaselineValues((b) => ({ ...b, [a.name]: { mean: "", rangeLow: "", rangeHigh: "", target: "", ...b[a.name], sd: e.target.value } }))} style={{ ...inputStyle, width: 70, marginTop: 0 }} />
                    <span style={{ fontSize: 10, color: "#C7D1CE" }}>·</span>
                    <input placeholder="Target (optional)" type="number" value={baselineValues[a.name]?.target ?? ""} onChange={(e) => setBaselineValues((b) => ({ ...b, [a.name]: { mean: "", sd: "", rangeLow: "", rangeHigh: "", ...b[a.name], target: e.target.value } }))} style={{ ...inputStyle, width: 90, marginTop: 0 }} />
                  </div>
                ))}
