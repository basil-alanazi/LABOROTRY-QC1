...
// Reads an .xlsx/.xls/.csv (via SheetJS) or .docx (via mammoth, table or
// "Label: value" line fallback) into a grid of rows (array of arrays).
...
export default function RecordModule({ table, title, description, fields, role, username }) {
  const [rows, setRows] = useState(null);
  const [showAdd, setShowAdd] = useState(false);
  const [form, setForm] = useState(() => Object.fromEntries(fields.map((f) => [f.key, f.type === "date" ? new Date().toISOString().slice(0, 10) : ""])));
  ...
  async function addRow() {
    ...
    setForm(Object.fromEntries(fields.map((f) => [f.key, f.type === "date" ? new Date().toISOString().slice(0, 10) : ""])));
    setShowAdd(false);
    loadAll();
  }
  ...
  async function exportExcel() {
    ...
    const aoa = [header, ...rows.map((r) => fields.map((f) => r[f.key] ?? ""))];
  ...
  async function importFile(e) {
    ...
    try {
      ...
      const newRows = dataRows.filter((r) => r.some((c) => String(c ?? "").trim() !== "")).map((r) => {
        const rec = {};
        fields.forEach((f, i) => {
          const mapped = fieldIndexes.find((x) => x.key === f.key);
          const idx = looksMapped ? mapped.idx : i;
          rec[f.key] = idx !== -1 && idx !== undefined ? String(r[idx] ?? "").trim() : "";
        });
        return rec;
            ...
            <tbody>
              {rows.map((r) => (
                <tr key={r.id} style={{ borderTop: "1px solid #EEF2F0" }}>
                  {fields.map((f) => <td key={f.key} style={{ padding: "6px 10px", borderRight: "1px solid #EEF2F0" }}>{r[f.key]}</td>)}
